...
You can mathematically verify this receipt off-chain by using the Accensa Node SDK:

```typescript
import { verifyReceipt } from '@accensa/sdk';

const isValid = verifyReceipt(receipt, proof, batchRoot);
if (!isValid) {
  throw new Error("Invalid receipt signature or tampered metadata.");
}
```
